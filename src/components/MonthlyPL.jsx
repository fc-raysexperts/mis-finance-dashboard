  ...
  return (
    <>
      ...
      <tr className="annual-head-row head-group-end">
        <td className="col-sub annual-head-label">Depreciation</td>
        {depnVals.map((v, i) => (
          <td key={i} className={`col-num${v < 0 ? ' neg' : ''}`}>{fmt(v)}</td>
        ))}
      </tr>
      <tr className="annual-profit-row">
        <td className="col-sub">Profit Before Dep &amp; Tax</td>
        {profitVals.map((v, i) => (
...
export default function MonthlyPL({ revData, expData, visMo }) {
  ...
  const rows = [
    ...
    { label: 'Total Expenses (Excl. Tax)', arr: totalExp, cls: 'pl-exp' },
    { label: 'Finance Cost (Interest)',    arr: finCost,  cls: 'pl-sub' },
    { label: 'Depreciation',              arr: depn,     cls: 'pl-sub' },
    { label: 'Profit Before Dep & Tax',   arr: profit,   cls: 'pl-profit', isProfit: true },
  ];
