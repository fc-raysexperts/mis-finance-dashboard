...
export default async function handler(req, res) {
  ...
  // Channel 3 now stores CWIP, IAUD, and land lease registration NEW NPD
  // as three separate keys (not one combined "Total Till Date" key), so
  // each can be checked and recomputed independently by the dedicated
  // refresh endpoint.
  keysToDelete.push('npd:cache:generic_accounts_txns:CWIP');
  keysToDelete.push('npd:cache:generic_accounts_txns:IAUD');
  keysToDelete.push('npd:cache:generic_accounts_txns:LLR');
